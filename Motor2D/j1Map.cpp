...
bool j1Map::Load(const char* file_name)
{
	...
	for (layer = map_file.child("map").child("layer"); layer && ret; layer = layer.next_sibling("layer"))
	{
		...
		if (ret == true)
		{
			ret = LoadLayerDetails(layer, set);
		}
...
bool j1Map::LoadLayerDetails(pugi::xml_node& layer_node, Layer* set)
{
	set->height	= layer_node.attribute("height").as_uint();

	set->width = layer_node.attribute("width").as_uint();

	set->name	= layer_node.attribute("name").as_string();
	...
	{
		set->tiles[i] = tile_node.attribute("gid").as_uint();
